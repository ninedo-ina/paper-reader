...
发布分支：`feature/v0.1.27`；目标：修正 PDF 空白区域拖拽事件与抓手光标状态。

- 悬停空白区域显示 `grab`，不需要点击才能进入抓手状态。
- 仅在空白区域 `mousedown` 后进入 `grabbing`，使用原生 `window mousemove/mouseup` 保证拖拽过程中不丢失事件。
- 文本层、批注层、链接和控件不启动平移，保留文本选择。
- `mouseup`、窗口失焦和组件卸载都会清理拖拽状态。
