...
- 当前发布版本：`0.1.50`（侧栏书架四个 tab 的计数徽标改为标题角标，`REQ-202609-0126`，2026-09-17 UTC 已发布）
- 当前发布版本分支：`feature/v0.1.50`；生产前端当前运行 `0.1.50`（2026-09-17 UTC 已核验）。**后端本轮为纯前端改动、未重新构建也未重启，线上仍是 `paper-reader-backend-0.1.49.jar`**，故 `/api/health` 仍返回 `0.1.49`，判断前端版本请看 favicon `?v=` 与构建产物哈希
- 开发中版本：无（`0.1.50` 已发布，下一迭代尚未开始）
- 长期分支：`dev`（集成）、`main`（默认/生产）。本轮按需求直接 `feature/v0.1.50 -> main`，`dev` 仍停在 `b0409ef`（是 `main` 的祖先）
