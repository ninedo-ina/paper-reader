...
> 当前开发版本：`0.1.48`（国际化语言切换，`REQ-202609-0111`，已发布，无后续迭代在开发）；当前生产版本：`0.1.48`（2026-09-16 UTC 已核验，`/api/health` 返回 `0.1.48`，登录页语言下拉可选 14 种语言并整页切换、`ar`/`fa`/`ug` 为 RTL，Flyway 仍在 `V15`）；核对日期：2026-09-16（UTC）；生产域名：`https://paper.pilo.eu.cc`
...
- 集成分支：`dev`。
- 当前发布分支：`feature/v0.1.48`（国际化语言切换，`REQ-202609-0111`，提交 `af63e4d`），从已发布基线 `v0.1.47` 展开，已按 `dev -> main` 流程合并并部署，**当前没有在开发的下一迭代**。
- 上一发布分支：`feature/v0.1.47`（登录后的默认身份与个人中心邮箱，`REQ-202609-0110`，提交 `257872c`），已按 `dev -> main` 流程合并。
- `0.1.34` ~ `0.1.48` 为一串连续的小版本迭代（阅读器交互、两步验证、信任设备、UI 打磨、接入通知中心、多语言与登录身份等），均已按 `feature/vX.Y.Z -> dev -> main` 发布，逐条记录见 `docs/MAINTENANCE.md`。本节此前停留在 `0.1.32`，属交接文档未随发布更新，已于 2026-09-15 校正。
...
当前历史回归基线：前端 8 个测试文件、66 项测试；后端 33 项测试。该数字是 v0.1.23 时期的记录，只作历史参考；每轮迭代都必须重新运行全套检查，以当轮实测结果为准（v0.1.48 前端为 20 个测试文件、121 项测试，后端 73 项）。
