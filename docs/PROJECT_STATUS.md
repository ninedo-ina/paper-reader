...
> 当前发布版本：`0.1.47`；发布分支：`feature/v0.1.47`；当前生产版本：`0.1.47`（2026-09-15 UTC 已核验，`/api/health` 返回 `0.1.47`；本轮含 Flyway `V15`，已在生产执行，`pr_users.avatar_url` 放宽到 `VARCHAR(1000000)`，登录后个人中心显示默认用户名与邮箱）；核对日期：2026-09-15（UTC）；生产域名：`https://paper.pilo.eu.cc`
...
- 集成分支：`dev`。
- 当前开发分支：`feature/v0.1.47`（登录后的默认身份与个人中心邮箱，`REQ-202609-0110`），在已发布基线 `v0.1.46` 上开发。
- 上一发布分支：`feature/v0.1.46`，已按 `dev -> main` 流程合并。
- `0.1.34` ~ `0.1.47` 为一串连续的小版本迭代（阅读器交互、两步验证、信任设备、UI 打磨、接入通知中心、多语言与登录身份等），均已按 `feature/vX.Y.Z -> dev -> main` 发布，逐条记录见 `docs/MAINTENANCE.md`。本节此前停留在 `0.1.32`，属交接文档未随发布更新，已于 2026-09-15 校正。
