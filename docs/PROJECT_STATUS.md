...
> 当前开发版本：`0.1.50`（侧栏书架四个 tab 的计数徽标改为标题角标，`REQ-202609-0126`，已发布，无后续迭代在开发）；当前生产版本：**前端 `0.1.50` / 后端 `0.1.49`**（2026-09-17 UTC 已核验：favicon 为 `?v=0.1.50`，服务端样式表与全部首屏 JS chunk 与本地构建逐字节一致，Flyway 仍在 `V15`；`REQ-202609-0126` 是纯前端改动，后端 jar 有意未重建、未重启，故 `/api/health` 仍返回 `0.1.49`，判断前端版本请看 favicon `?v=` 与构建产物哈希）；核对日期：2026-09-17（UTC）；生产域名：`https://paper.pilo.eu.cc`
...
- 集成分支：`dev`。
- 当前发布分支：`feature/v0.1.50`（侧栏书架四个 tab 的计数徽标改为标题角标，`REQ-202609-0126`，提交 `8f47696`/`fac6c9e`/`a616068`），从已发布基线 `b0409ef`（= v0.1.49 的 `main`）展开，以显式 `--no-ff` 合并提交 `658c3ba` 合入 `main` 并部署，**当前没有在开发的下一迭代**。本轮**按需求直接 `feature/v0.1.50 -> main`，未经过 `dev`**；`dev`（本地与 `origin/dev`）仍停在 `b0409ef`，是 `main` 的祖先、未分叉。
- 上一发布分支：`feature/v0.1.49`（语言面板隐藏滚动条但保留滚动，`REQ-202609-0111` 收尾，提交 `dc6b9c6`/`6df5bd5`），已按 `dev -> main` 流程快进合并。
- 上上发布分支：`feature/v0.1.48`（国际化语言切换与登录页语言下拉修复，`REQ-202609-0111`，提交 `af63e4d`/`00d2351`），已按 `dev -> main` 流程合并。
- `0.1.34` ~ `0.1.50` 为一串连续的小版本迭代（阅读器交互、两步验证、信任设备、UI 打磨、接入通知中心、多语言与登录身份、侧栏徽标版式等），除 `0.1.50` 外均按 `feature/vX.Y.Z -> dev -> main` 发布，逐条记录见 `docs/MAINTENANCE.md`。本节此前停留在 `0.1.32`，属交接文档未随发布更新，已于 2026-09-15 校正。
...
- 除 `zh` / `en` 外的 12 种语言文案来自批量机翻，术语一致性未经人工校对；后端返回的 `message` 仍是中文，不随语言切换。
...
当前历史回归基线：前端 8 个测试文件、66 项测试；后端 33 项测试。该数字是 v0.1.23 时期的记录，只作历史参考；每轮迭代都必须重新运行全套检查，以当轮实测结果为准（v0.1.50 前端为 22 个测试文件、129 项测试，后端 73 项未变）。
...
- 分支：`feature/v0.1.31`。
- 需求：中文侧栏 `nav.created` 从“我的创作”改为“我的论文”，英文从 “My Creations” 改为 “My Papers”。
- 内部路由键 `created`、书架 Tab 键 `create`、图标和业务逻辑保持不变。
- 前后端版本及 favicon 缓存参数已升级到 `0.1.31`。不涉及数据库迁移、后端接口、用户数据或共享基础设施。
- 状态：已完成测试、提交、推送、合并、生产部署和线上验收。
