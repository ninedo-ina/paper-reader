...
> 当前发布版本：`0.1.45`；发布分支：`feature/v0.1.45`（提交 `f1fa061`）；当前生产版本：`0.1.45`（2026-09-15 UTC 已核验，`/api/health` 返回 `0.1.45`，登录页内容块在 2560×1440 下垂直居中；上一版 `0.1.44` 的登录验证码邮件已在线收到）；核对日期：2026-09-15（UTC）；生产域名：`https://paper.pilo.eu.cc`
...
- 集成分支：`dev`。
- 当前开发分支：`feature/v0.1.45`（登录页垂直居中，`REQ-202609-0106`），在已发布基线 `v0.1.44` 上开发。
- 上一发布分支：`feature/v0.1.44`（接入通知中心，`REQ-202609-0107`），已按 `dev -> main` 流程合并。
- `0.1.34` ~ `0.1.44` 为一串连续的小版本迭代（阅读器交互、两步验证、信任设备、UI 打磨、接入通知中心等），均已按 `feature/vX.Y.Z -> dev -> main` 发布，逐条记录见 `docs/MAINTENANCE.md`。本节此前停留在 `0.1.32`，属交接文档未随发布更新，已于 2026-09-15 校正。
