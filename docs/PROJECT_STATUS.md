...
> 当前开发版本：`0.1.51`（菜单栏计数回退为行内胶囊、仅侧栏 `TabBar` 保留角标，`REQ-202609-0126` 的范围纠正，已发布，无后续迭代在开发）；当前生产版本：**前端 `0.1.51` / 后端 `0.1.49`**（2026-09-17 UTC 已核验：favicon 为 `?v=0.1.51`，服务端样式表与 `[locale]/page-*.js` chunk 与本地构建逐字节一致，Flyway 仍在 `V15`；`REQ-202609-0126` 是纯前端改动，后端 jar 有意未重建、未重启，故 `/api/health` 仍返回 `0.1.49`，判断前端版本请看 favicon `?v=` 与构建产物哈希）；核对日期：2026-09-17（UTC）；生产域名：`https://paper.pilo.eu.cc`
...
- 集成分支：`dev`。
- 当前发布分支：`feature/v0.1.51`（菜单栏计数回退为行内胶囊、仅侧栏 `TabBar` 保留角标，`REQ-202609-0126` 范围纠正，提交 `8331e47`/`f3ff2be`），从 v0.1.50 的发布提交 `f2f708e` 展开，以显式 `--no-ff` 合并提交 `cd46367` 合入 `main` 并部署，随后 `main` 又接受一个仅文档的同步提交，`dev` 从 `f2f708e` 快进到与 `main` 相同的提交（先到 `cd46367`，再跟到该文档提交），三个分支重新对齐，**当前没有在开发的下一迭代**。
...
当前历史回归基线：前端 8 个测试文件、66 项测试；后端 33 项测试。该数字是 v0.1.23 时期的记录，只作历史参考；每轮迭代都必须重新运行全套检查，以当轮实测结果为准（v0.1.50 起前端为 22 个测试文件、129 项测试，v0.1.51 未变，后端 73 项未变）。
