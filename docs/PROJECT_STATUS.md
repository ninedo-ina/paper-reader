...
> 当前开发版本：`0.1.48`（国际化语言切换，`REQ-202609-0111`，开发中）；当前生产版本：`0.1.47`（2026-09-15 UTC 已核验，`/api/health` 返回 `0.1.47`，登录后个人中心显示默认用户名与邮箱、`pr_users.avatar_url` 放宽到 `VARCHAR(1000000)`，Flyway 已到 `V15`）；核对日期：2026-09-15（UTC）；生产域名：`https://paper.pilo.eu.cc`
...
- 集成分支：`dev`。
- 当前开发分支：`feature/v0.1.48`（国际化语言切换，`REQ-202609-0111`），在已发布基线 `v0.1.47` 上开发。
...
当前历史回归基线：前端 8 个测试文件、66 项测试；后端 33 项测试。该数字是 v0.1.23 时期的记录，只作历史参考；每轮迭代都必须重新运行全套检查，以当轮实测结果为准（v0.1.48 前端为 20 个测试文件、120 项测试）。
