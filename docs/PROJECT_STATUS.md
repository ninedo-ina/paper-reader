...
- 集成分支：`dev`。
- 当前发布分支：`feature/v0.1.51`（菜单栏计数回退为行内胶囊、仅侧栏 `TabBar` 保留角标，`REQ-202609-0126` 范围纠正，提交 `8331e47`/`f3ff2be`），从 v0.1.50 的发布提交 `f2f708e` 展开，以显式 `--no-ff` 合并提交 `cd46367` 合入 `main` 并部署，并把 `dev` 从 `f2f708e` 快进到 `cd46367`，**当前没有在开发的下一迭代**。
