...
- 版本链：`8331e47`（回退 + `ATTENTION.md` 更正）→ `f3ff2be`（版本号升到 `0.1.51`）→ `13421c6` 等仅文档的同步提交，均在 `feature/v0.1.51` 并推送；以**显式 `--no-ff` 合并提交** `cd46367`（父提交 `f2f708e` + `f3ff2be`）合入 `main` 并推送，随后把 `dev` 从 `f2f708e` **快进**到 `cd46367`（非强推、未改写历史）；文档同步提交随后以 `--ff-only` 进入 `main`，`dev` 一并快进到同一提交，至此 `main` / `dev` / `feature/v0.1.51` 三点对齐（三者同指 `feature/v0.1.51` 的最后一个提交）。
