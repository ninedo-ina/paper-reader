...
- 遗留与人工事项：① 线上后端仍是 `paper-reader-backend-0.1.49.jar`（有意，见「构建与部署」），下次后端有实际改动时会随新 jar 把版本号带到线上；② `dev` 未跟进本轮（仍为 `b0409ef`，是 `main` 的祖先，v0.1.51 里已快进到 `cd46367`）；③ 侧栏折叠态的圆形菜单项徽标本轮未改动，也未纳入本次几何验收；④ 版式实测依赖 mock 计数，登录态下用真实计数的截图验收未做；⑤ v0.1.48 / v0.1.49 的遗留（机翻未校对、中文 AI 提示词与后端 `message` 有意保留）不变；⑥ **本节把菜单栏 `Sidebar.tsx` 一并改掉属越界，已由 v0.1.51 回退，见下条**——本节「实现」「测试」「线上验收」里凡涉及 `Sidebar.tsx` / `ml-auto` / 两处 `aria-label` 的表述均已失效。
...
- 版本链：`8331e47`（回退 + `ATTENTION.md` 更正）→ `f3ff2be`（版本号升到 `0.1.51`），均在 `feature/v0.1.51` 并推送；以**显式 `--no-ff` 合并提交** `cd46367`（父提交 `f2f708e` + `f3ff2be`）合入 `main` 并推送，随后把 `dev` 从 `f2f708e` **快进**到 `cd46367`（非强推、未改写历史），至此 `main` / `dev` / `feature/v0.1.51` 三点对齐。
