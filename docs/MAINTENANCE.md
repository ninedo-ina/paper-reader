...
- 遗留与人工事项：① 线上后端仍是 `paper-reader-backend-0.1.49.jar`（有意，见「构建与部署」），下次后端有实际改动时会随新 jar 把版本号带到线上；② `dev` 未跟进本轮（仍为 `b0409ef`，是 `main` 的祖先）；③ 侧栏折叠态的圆形菜单项徽标本轮未改动，也未纳入本次几何验收；④ 版式实测依赖 mock 计数，登录态下用真实计数的截图验收未做；⑤ v0.1.48 / v0.1.49 的遗留（机翻未校对、中文 AI 提示词与后端 `message` 有意保留）不变。
