...
当前已发布版本为 `0.1.45`（分支 `feature/v0.1.45`，2026-09-15 UTC 已合入 `dev` / `main` 并完成生产部署与线上验收）；后续新需求从该基线创建新分支（本轮之后为 `feature/v0.1.48`）。v0.1.33 之后本项目实际按普通迭代分支（`feature/v0.1.40`、`feature/v0.1.41`、`feature/v0.1.42`、`feature/v0.1.43`、`feature/v0.1.44`、`feature/v0.1.45`）递进，不再追加 `-fix`，即使本轮只是补丁也照常开下一个修订号分支；历史 `-fix` 分支仅用于 v0.1.32 及更早版本。历史版本分支全部保留，不以早期初始化基线替代当前 `dev`。
...
### v0.1.39 登录页轮播区右移下移（2026-09-14 UTC）

- 需求（`REQ-202609-0103`）：左侧轮播展示区位置不好看——整体往右靠一点、往下挪一点；右侧登录表单位置保持不动。
- 这是对 v0.1.36/v0.1.37「轮播上下边界与登录卡片严格对齐」的有意调整：用户反馈顶到卡片顶边偏生硬，改为整体内缩后与更高的卡片形成视觉平衡。
...
### v0.1.47 登录后的默认身份、个人中心邮箱与头像菜单收起（2026-09-15 UTC）

- 需求（`REQ-202609-0110`）：登录后账号没有默认头像、没有默认用户名，个人中心看不到邮箱，头像设置弹窗点空白处不收起。
- 根因（关键）：**不是个人中心渲染的问题，是登录链路没取资料**。`SessionLoader` 只在整页加载时跑一次；邮箱验证码登录、GitHub 登录成功后走的是客户端路由跳转，根布局不会重挂载，`useUserStore.profile` 一直是 `null` —— 所以顶栏/个人中心的用户名、邮箱、头像全部取不到（头像退化成「?」）。修复放在 `frontend/src/stores/auth-store.ts` 的 `applyTokens` 成功分支末尾调用 `useUserStore.getState().loadProfile()`：`applyTokens` 是三条登录路径（邮箱验证码 / GitHub / 账号密码）的唯一汇聚点，改动面最小。`loadProfile` 内部加了 `inflight` 去重，重复调用只会发一个请求。
- 默认身份：新增 `frontend/src/lib/user-display.ts` 作为唯一出处 —— `avatarInitial()` 取邮箱首字符（ASCII 字母转大写，数字/CJK 原样，兜底 `?`）；`avatarBackground()` 用 31 乘子哈希把邮箱映射到 12 色纯色盘；`avatarForeground()` 按 WCAG 2.1 相对亮度算对比度，在白 `#ffffff` 与深墨 `#1f2933` 里挑对比度更高的一个（保证 ≥ 4.5:1，不会出现同色糊在一起）；`defaultDisplayName()` 依次回退 `displayName` → `用户{id}` → 邮箱本地部分 → `用户`。个人中心、顶栏 `UserMenu`、聊天 `ChatPanel` 三处统一用它，不再各写一份。**注意头像字符按需求取邮箱首字符，不是用户名首字符**（需求原文就是邮箱）。
- 头像菜单收起：`ProfileDialog` 的 `AvatarSection` 在菜单展开时挂 `document` 上的 `mousedown` 监听，点到 `menuRef`（菜单本体）和 `avatarRef`（触发按钮）之外的任何地方即关闭并同时复位 URL 输入态；另加 `Escape` 关闭。触发按钮要单独豁免，否则 `mousedown` 先关、`onClick` 再开，表现成「点了没反应」。
- 后端邮箱补全：GitHub 授权虽然申请了 `scope=user:email`，但一直只读 `/user` 返回的 `email`（可能是 `null`），从没请求过 `/user/emails`。新增 `fetchGithubPrimaryEmail()`（`GET https://api.github.com/user/emails`，`ParameterizedTypeReference<List<Map<String, Any>>>` 接数组；优先 primary+verified → 任一 verified → 第一条），`fetchGithubUser` 拿不到邮箱时回退到它；同时新增 `backfillGithubEmail()`，老账号登录时把 `@github.user` 占位邮箱换成真邮箱。占位域名常量 `GITHUB_PLACEHOLDER_DOMAIN = "@github.user"`。**冲突保护**：若真邮箱已被另一个账号占用，保留占位邮箱并记 WARN，不做合并、不抛异常。邮箱验证码 / 账号密码新注册用户的 `displayName` 改为 `null`（交给 `defaultDisplayName` 生成「用户{id}」），不再把邮箱前缀当默认用户名。
- 数据库迁移 `V15__widen_user_avatar.sql`：`pr_users.avatar_url` 由 `VARCHAR(500)` 放宽到 `VARCHAR(1000000)`。原宽度装不下 base64 data URL（500 字符 ≈ 368 字节图片），上传头像会直接写库失败。实体 `User.kt` 同步加 `@Column(name = "avatar_url", length = 1_000_000)`，前端上传前按 900 000 字符截断兜底（留 JSON 转义余量）。**必须保持 varchar**：生产 `ddl-auto=validate`，改成 `text` 会因类型不匹配启动失败。
- 并行协作：同期另有 `REQ-202609-0111`（多语言，分支 `feature/v0.1.46`，另一个 worktree `/root/paper-reader.wt-req-202609-0111`）在同一仓库上工作。截至本次合并，`feature/v0.1.46` 一直是**未提交状态**（停留在 `f5c8838`），因此本次从 `f5c8838` 直接快进，不存在合并冲突。两边重叠文件只有 `frontend/src/app/layout.tsx`（favicon `?v=`）与 `frontend/src/components/settings/ProfileDialog.tsx`（对方在做 i18n）。若对方之后 rebase，需保留其 i18n 结构并把本轮的默认头像/点击外部收起逻辑串进去，`profile-dialog.test.tsx` 要包一层对方的 `src/test/intl.tsx` 的 `withIntl`。
- 测试：新增 `frontend/src/test/user-display.test.ts` 8 项（含对比度下限与回退链）与 `frontend/src/test/profile-dialog.test.tsx` 6 项（断言头像取邮箱首字符 `S`、默认用户名 `用户8`、点 body 关闭、按 Escape 关闭、点菜单内部不关）；`AuthServiceTest.kt` 新增 4 项 GitHub 邮箱用例（`/user/emails` 回退、占位邮箱回填、真邮箱被占用时保留占位并 WARN、本地部分不变）。前端 18 个文件 105 项测试全过，`tsc --noEmit` 与 `next lint` 退出码 0、`next build` 通过；后端 73 项测试全绿（`./gradlew clean test bootJar` `BUILD SUCCESSFUL`），产出 `paper-reader-backend-0.1.47.jar`。
- 版本链：`257872c`（功能）与 `acac3e5`（版本号 + 迭代文档，`feature/v0.1.47`）→ 快进合入 `dev` 与 `main`（均为 `f5c8838..acac3e5`），非强推、非改写历史。同样因为 `main` 被生产目录 `/root/paper-reader` 检出，`dev -> main` 在 `/root/paper-reader` 内以 `merge --ff-only` 完成。
- 构建与部署：前后端 VERSION、`package.json`、`build.gradle.kts`、favicon `?v=` 统一升到 `0.1.47`；按 `DEPLOY.md` 同一 shell 内 `set -a; . ./.env; set +a` → `pm2 delete paper-reader-backend` → 用新 JAR 绝对路径 `pm2 start` 重建后端（**id 由 8 变 9**）、`pm2 restart paper-reader-frontend --update-env`（重启计数 8）后 `pm2 save`。启动日志 `Successfully validated 15 migrations` / `Current version of schema "public": 14` / `Migrating schema "public" to version "15 - widen user avatar"` / `Successfully applied 1 migration ... now at version v15` / `Started PaperReaderApplicationKt in 8.592 seconds`。
- 线上验收（2026-09-15 UTC，`https://paper.pilo.eu.cc`）：`/api/health` 与本机 `127.0.0.1:8080/api/health` 均返回 `{"status":"ok","version":"0.1.47"}`；`/zh/login` 与 `http://localhost:3001/zh/login` 均 200，服务端 HTML favicon 为 `icon-light.svg?v=0.1.47`；生产 `_next/static/chunks/app/%5Blocale%5D/page-c6aff573d06395a7.js` 与本地构建 sha256 逐字节一致（`797d9ed3…c01281`），其中已能搜到「更换头像」「从本地上传」「未绑定邮箱」「2f6f9f」等本轮新串，确认新版确实进了线上产物；主 chunk `759-bfbe222f0849ee12.js` 同样 sha256 一致。
- 数据库校验：`pr_users.avatar_url` 的 `character_maximum_length` 已由 500 变为 **1000000**；`flyway_schema_history` 最新两行为 `15|widen user avatar|t` 与 `14|two factor and trusted devices|t`。
- 本轮无新依赖、无接口出入参变化；**唯一的破坏性变更风险在前述并行分支**：V15 已在生产执行，若之后部署一个不含 `V15__widen_user_avatar.sql` 的版本，Flyway 会以 `Detected applied migration not resolved locally` 拒绝启动，必须先确保该文件在分支上存在。

### v0.1.48 国际化语言切换（14 种语言）与登录页语言下拉修复（2026-09-16 UTC）

- 需求（`REQ-202609-0111`）：界面支持简体中文（默认）、繁体中文、藏语、维吾尔语、德语、阿拉伯语、韩文、日文、法语、越南语、西班牙语、意大利语、波斯语共 13 种（外加线上原有的 `en`，共 14 种）；登录页的语言切换按钮点了没反应，要改成点开下拉列表选语言；登录后系统跟随所选语言；个人设置 → 偏好设置新增「语言」项，并把当前选择的语言写入其中。
- 语言注册表：新增 `frontend/src/i18n/locales.ts`，`LOCALES` / `LOCALE_META`（母语名、英文名、书写方向）是唯一来源，切换器、设置页、`<html lang>/<html dir>`、浏览器语言协商都从它读；**加语言只改这里 + 新增一份 `locales/<code>/common.json`**。RTL 语言为 `ar` / `fa` / `ug`。
- 文案：14 份 `frontend/src/i18n/locales/<code>/common.json`，逐条对齐，拍平后各 833 条可翻译条目（叶子键 691 个），0 缺 0 多。批量翻译走 `/tmp` 下的脚本（不入库、经外部代理、有 429 限流），**机器翻译未经人工校对**，`ar`/`bo`/`ug`/`fa` 等低资源语言建议后续对高频页面做一轮人工校订。
- 切换链路：`applyLocale()`（`src/i18n/switch-locale.ts`）写 `NEXT_LOCALE` Cookie + `localStorage.paperhelper.locale-chosen` 标记，然后 **`window.location.assign()` 整页跳转**；不能改成 `router.push` —— 根布局 `app/layout.tsx` 在 `app/[locale]/` 之上，软导航不会重渲染它，`<html lang/dir>` 与消息表都会停在旧语言。
- 账号偏好：登录后选语言会 `PUT /api/settings` 写 `UserSettings.language`；新设备登录时若本机没有「用户亲自选过」的标记，就采用账号偏好。判断「用户选过」只看 localStorage 标记，不看 Cookie（next-intl 每次请求都会写 Cookie）。设置页新增 `LanguagePicker.tsx`。
- 组件外文案（API 客户端、AI provider 与响应解析、Zustand store、设备工具）走 `src/i18n/runtime.ts` 的 `runtimeTranslator()`，消息表由根布局的 `RuntimeLocaleBridge` 在渲染期登记；**取文案必须写在调用的函数里，不能提到模块顶层做常量**。
- 登录页开关失效有**两个独立根因**：一是软导航不重渲染根布局（上一轮改成整页跳转）；二是**层叠**——下拉挂在顶栏里，顶栏与登录内容行是同为 `z-10` 的兄弟，内容行在 DOM 里更靠后，兄弟同 `z-index` 时后者赢，于是菜单被登录卡片压住，落在卡片范围内的选项既看不见也点不着（线上 `elementFromPoint` 实测命中 `DIV.space-y-5 px-8 py-2`）。修复：`(auth)/layout.tsx` 顶栏提到 `relative z-30`（内容行/页脚仍 `z-10`），新增回归用例 `frontend/src/test/auth-layout.test.tsx` 断言 `zIndexOf(header) > zIndexOf(row)`，改回 `z-10` 会以 `expected 10 to be greater than 10` 失败。
- RTL：根布局按 `LOCALE_META[locale].dir` 写 `<html dir>`，布局改用 Tailwind 逻辑方向工具类（`ms-`/`me-`/`ps-`/`pe-`/`start-`/`end-`/`text-start`/`border-e`）。
- 测试：前端 20 个文件 121 项全过（新增 `auth-layout.test.tsx` 层叠用例）；`npx tsc --noEmit`、`next lint`（仅剩既有的 `no-img-element` 告警）、`next build` 均退出码 0；后端 73 项测试全绿，`./gradlew clean test bootJar` `BUILD SUCCESSFUL`。
- 版本链：`ed845f1`（14 种语言与全量文案国际化）→ `a73ba3d`（版本号升到 `0.1.48`、上轮新增中文并入 i18n、迭代文档）→ `af63e4d`（登录页下拉层叠修复）→ `957085e`（层叠回归用例），均在 `feature/v0.1.48`；随后 `feature/v0.1.48 -> dev -> main` 全部快进合并并推送（非强推、未改写历史）。`main` 被生产目录 `/root/paper-reader` 检出，故 `dev -> main` 在 `/root/paper-reader` 内以 `merge --ff-only af63e4d` 完成。本轮还删掉了被取代的自建远程分支 `feature/v0.1.46`（只有本轮用过）。
- 构建与部署：前后端 VERSION、`frontend/package.json`、`backend/build.gradle.kts`、favicon `?v=` 统一到 `0.1.48`。后端本轮 **Kotlin 代码零改动**，仍按 `DEPLOY.md` 重新 `./gradlew clean test bootJar`（产出 `paper-reader-backend-0.1.48.jar`），在同 shell 内 `set -a; . ./.env; set +a` → `pm2 delete paper-reader-backend` → 用新 JAR 绝对路径 `pm2 start` 重建后端（**id 由 9 变 10**）；前端 `pnpm run build` 后 `pm2 restart paper-reader-frontend --update-env`（重启计数 10），再 `pm2 save`。后端启动日志：`Successfully validated 15 migrations` / `Current version of schema "public": 15` / `Schema "public" is up to date`，未新增迁移。
- 线上验收（2026-09-16 UTC，`https://paper.pilo.eu.cc`）：`/api/health` 返回 `{"status":"ok","version":"0.1.48"}`；`/zh|zh-Hant|bo|ug|ar|de|ja|fr/login` 全部 200，`ug`/`ar` 的 `<html dir="rtl">`、`bo` 为 `ltr`，favicon 为 `paperhelper-favicon-light.svg?v=0.1.48`；服务端 HTML 中顶栏类名为 `relative z-30 mx-auto flex w-full max-w-7xl …`；公网 `_next/static/chunks/app/%5Blocale%5D/(auth)/layout-9fa49e62959180d4.js` 与本地 `.next` 构建 sha256 逐字节一致（`8ace9f45f7d9b9cb…`）。Playwright 对公网登录页实测：点开语言按钮得到 14 项（母语名 + 英文名），点「العربية」跳 `/ar/login`（`dir=rtl`）、点「فارسی」跳 `/fa/login`（`rtl`）、点「བོད་སྐད་」跳 `/bo/login`（`ltr`）；对与登录卡片重叠的可见选项做 `elementFromPoint` 命中检测，命中的是菜单自身而非卡片，截图确认下拉整体压在卡片之上。
- 遗留与人工事项：① 13 种语言（除 `en`/`zh`）为机器翻译，未做人工校对；② `src/stores/chat-store.ts` 里的 AI 系统提示词与后端返回的 `message` 仍为中文，属有意保留（提示词要求模型跟随用户语言回答，后端错误应改走错误码映射）；③ 本轮后端接口、数据库、依赖均无改动，未做未登录用户的完整登录端到端验收（语言切换本身在未登录的登录页已验证）。

### v0.1.49 语言面板隐藏滚动条但保留滚动（2026-09-16 UTC）

- 需求（`REQ-202609-0111` 收尾）：登录页点语言图标弹出的面板上有一条滚动条，用户要求去掉滚动条但保留滚动，并确认其它语言选择面板是否同样如此。**两个面板都有**：`LanguageSwitcher` 的下拉（`max-h-80`）和设置页/偏好设置共用的 `LanguagePicker`（`max-h-72`，网格两列）——14 种语言一定放不下，`overflow-y-auto` 不能去掉，而 `globals.css` 里那条全局 `::-webkit-scrollbar { width: 6px }`（`--text-tertiary`）只要真的溢出就会贴着面板右边缘切进圆角。修完两个都不露了。
- 实现：新增工具类 `.scrollbar-hidden`（定义在 `frontend/src/app/globals.css`，紧跟全局 `::-webkit-scrollbar` 那段），三条声明缺一不可 —— `scrollbar-width: none`（Firefox / Chrome 121+）、`-ms-overflow-style: none`（旧 Edge）、`::-webkit-scrollbar { width: 0; height: 0; display: none }`（Chromium / Safari）。两处面板挂类，`overflow-y-auto` 原样保留，滚轮 / 触屏 / 键盘滚动不受影响；`LanguagePicker` 里给滚动条让位的 `pe-1` 一并去掉（滚动条没了，那条留白只会让网格整体偏左）。
- 测试：新增 `frontend/src/test/language-panel-scrollbar.test.tsx`（3 项）——两个面板各断言「有 `scrollbar-hidden` 且仍有 `overflow-y-auto`」，另一项直接读 `globals.css` 锁住那三条声明（jsdom 不加载样式表，只断言类名证明不了类有定义）。本文件同时约束了「不要顺手把 `overflow-y-auto` 一起删掉」。前端全量 21 个文件 124 项全绿，`npx tsc --noEmit`、`npm run lint`（仅既有的 `no-img-element` 告警）、`pnpm run build` 均退出码 0；后端 `./gradlew clean test bootJar` `BUILD SUCCESSFUL`，73 项测试 0 失败。
- 版本链：`dc6b9c6`（工具类 + 两处面板 + 回归用例）→ `6df5bd5`（版本号升到 `0.1.49`），均在 `feature/v0.1.49`；随后 `feature/v0.1.49 -> dev -> main` 快进合并并推送（非强推、未改写历史）。`main` 被生产目录 `/root/paper-reader` 检出，故 `dev -> main` 仍在 `/root/paper-reader` 内以 `merge --ff-only 6df5bd5` 完成。
- 构建与部署：前后端 VERSION、`frontend/package.json`、`backend/build.gradle.kts`、favicon `?v=` 统一到 `0.1.49`。后端本轮 **Kotlin 代码零改动**，仍按 `DEPLOY.md` 重新 `./gradlew clean test bootJar`（`paper-reader-backend-0.1.49.jar`），同 shell 内 `set -a; . ./.env; set +a` → `pm2 delete paper-reader-backend` → 用新 JAR 绝对路径 `pm2 start` 重建（**id 由 10 变 11**）；前端 `pnpm run build` 后 `pm2 restart paper-reader-frontend --update-env`（重启计数 11），健康检查通过后 `pm2 save`。后端启动日志：`Successfully validated 15 migrations` / `Current version of schema "public": 15` / `Schema "public" is up to date` / `Started PaperReaderApplicationKt in 8.624 seconds`，未新增迁移。
- 线上验收（2026-09-16 UTC，`https://paper.pilo.eu.cc`）：`/api/health` 返回 `{"status":"ok","version":"0.1.49"}`（本机 `127.0.0.1:8080` 同）；服务端 HTML favicon 为 `paperhelper-favicon-light.svg?v=0.1.49`；服务端样式表 `_next/static/css/d2f6bff4c5f568f7.css` 与本地构建 sha256 逐字节一致（`661aa40b…d9edfc`），其中 `scrollbar-hidden` 两条规则都在；含该类的三个 JS chunk（`745-a3e42f9d8657f3cd.js`、`[locale]/(auth)/layout-c528449574899750.js`、`[locale]/page-7564e7cd9e92d3f9.js`）与本地构建 sha256 亦逐字节一致。Playwright 对公网登录页实测：展开面板有 14 项，`getComputedStyle` 的 `scrollbar-width` 为 `none`、`::-webkit-scrollbar` 计算宽 `0px` / `display: none`，同时面板仍能滚（`scrollHeight 736 > clientHeight 318`，`scrollTop` 0 → 418）；**同页对照**（同样带边框的滚动容器、不带该类）的 `::-webkit-scrollbar` 是 `6px` / `display: inline`，即用户看到的那条。切成阿拉伯语后 `/ar/login`（`dir=rtl`）面板表现一致。
- 说明：`offsetWidth - clientWidth` 在本机 headless Chromium 里量不出滚动条宽度（该环境用叠加式滚动条，对照组同样是 0），所以验收改看 `::-webkit-scrollbar` 的计算样式与同页对照，**不要用像素差当这条的验收依据**。
- 遗留与人工事项：① 设置页的语言网格（`LanguagePicker`）在登录后才可达，本轮用单元测试 + 服务端样式表覆盖，未做登录态下的截图验收；② v0.1.48 的其余遗留（机翻未校对、中文 AI 提示词与后端 `message` 有意保留）不变。

### v0.1.50 侧栏书架四个 tab 的计数徽标改为标题角标（2026-09-17 UTC）

- 需求（`REQ-202609-0126`）：书架左栏的四个 tab（全部 / 创建 / 导入 / 收藏）一带上计数徽标，标题就被压成**逐字竖排**，读不出来。
- 根因：旧 `frontend/src/components/ui/TabBar.tsx` 里徽标是按钮内的**文档流内** `inline-flex` 兄弟节点，按钮本身 `flex-1` + `gap-1.5` + `px-3`（左右各 12px）。书架左栏 `aside.w-72` 只有 288px，四个等宽按钮实宽 71.75px，扣掉左右 padding 24px、徽标（`min-w-[18px]`，`99+` 实测约 24px）和 6px `gap-1.5` 后，留给标题的宽度只剩约 18px，而两个汉字需要 28px —— 于是标题逐字换行成 2 行（块高 40px、按钮高 60px）。`frontend/src/components/layout/Sidebar.tsx` 展开态的 `renderExpandedItem` 用的是同一套流内徽标 + `ml-auto`，同样会被压。**不是 CJK 专属问题**，逻辑上任何标签在足够窄的容器里都会中招（见下面 RTL 负对照）。
- 实现：徽标改为脱离文档流的角标 —— `absolute -top-2 -end-2`、`h-[16px] min-w-[16px]`（原为流内 `h-[18px] min-w-[18px]`）、`whitespace-nowrap`、`rounded-full` + 细边框、`aria-hidden="true"` + `pointer-events-none`（纯装饰，不该被点也不该被读屏）；计数 >99 截断成 `99+`，为 0 时不渲染。标题包一层 `span.relative inline-flex min-w-0 max-w-full`，内层 `span.min-w-0 truncate`，按钮去掉 `gap-1.5`。`Sidebar.tsx` 展开项同款角标，`flex-1 text-left` 与 `ml-auto` 一并换成 `min-w-0 truncate text-start`。定位一律用逻辑属性（`-end-2` / `-top-2` / `text-start`），LTR 落右上、RTL 自动镜像到左上。徽标 `aria-hidden` 之后按钮的可访问名会丢掉计数，两处都补了 `aria-label={badge ? \`${label} (${badge})\` : undefined}`。
- 测试：新增 `frontend/src/test/tabbar-badge.test.tsx`，锁定「徽标是脱离文档流的角标」而不是某段样式字符串——徽标必须 `absolute` / `-top-2` / `-end-2` / `pointer-events-none` / `aria-hidden="true"`，标题必须带 `truncate`，按钮 className **不得**再出现 `gap-1.5`（出现即意味着徽标又被塞回文档流），计数 `128` 截断为 `99+`、`0` 不渲染，可访问名为 `全部 (99+)`。前端全量 **22 个测试文件 129 项全绿**（上一版为 21 / 124）；`pnpm exec tsc --noEmit`、`pnpm lint`、`pnpm run build` 均退出码 0，lint 只剩既有告警（`no-img-element` 5 处、`ForumPage.tsx` 未使用的 `ChevronRight`/`ArrowLeft`、`PDFReader.tsx:260` 的 exhaustive-deps）。后端本轮零改动，未重跑。
- 版本链：`8f47696`（TabBar 与 Sidebar 的角标改造）→ `fac6c9e`（回归用例）→ `a616068`（版本号升到 `0.1.50`），均在 `feature/v0.1.50` 并推送 `origin/feature/v0.1.50`；随后以**显式 `--no-ff` 合并提交** `658c3ba`（父提交 `b0409ef` + `a616068`）合入 `main` 并推送（非强推、未改写历史）。本轮按需求直接走 `feature/v0.1.50 -> main`，**未经过 `dev`**：`dev`（本地与 `origin/dev`）仍停在 `b0409ef`，是 `main` 的祖先、未分叉，下一轮可从 `main` 开分支或先把 `dev` 快进到 `658c3ba`。
- 构建与部署：本轮是**纯前端改动**。前后端 `VERSION`、`frontend/package.json`、`backend/build.gradle.kts`、favicon `?v=` 按发布规范统一到 `0.1.50`；在 `/root/paper-reader/frontend` 执行 `pnpm run build` 后 `pm2 restart paper-reader-frontend`（重启计数 11 → 12，2026-09-17 08:56:05 UTC 起 online，进程 id 0 未变；重启前先做健康检查、`pm2 save`）。**后端 jar 未重新构建、`paper-reader-backend` 未重启**：线上后端进程 id 11 仍是 2026-09-16 00:37:01 UTC 起跑的 `paper-reader-backend-0.1.49.jar`（`restart_time=0`）。本轮 Kotlin 代码零改动，为一个不含任何变化的 jar 重启后端只会造成无收益的中断，因此**有意不重启**，属预期状态而非漏部署；代价是 `/api/health` 仍返回 `0.1.49`，判断前端版本请以 favicon `?v=` 与构建产物哈希为准。
- 线上验收（2026-09-17 UTC，`https://paper.pilo.eu.cc`）：
  - **构建产物逐字节一致**（证明线上跑的就是本地这一份，而不是只重启了进程）：服务端样式表 `_next/static/css/4693b861081067fc.css` sha256 `305d14ec22b8eb7b35611bd86093149ad7bee1d6307074445c12834639a1a956` 与本地 `.next` 完全相同，其中 `.-end-2{inset-inline-end:calc(var(--spacing) * -2)}`、`.-top-2{top:calc(var(--spacing) * -2)}`、`.text-start{text-align:start}`、`.min-w-\[16px\]{min-width:16px}` 都在，旧的 `.min-w-\[18px\]` 与 `.ml-auto` 已随旧徽标一起从构建中消失；`/zh` 首屏引用的 12 个 JS chunk 全部与本地构建 sha256 逐字节一致，其中 TabBar/Sidebar 所在的应用页 chunk `_next/static/chunks/app/%5Blocale%5D/page-2f621142043c6867.js` sha256 为 `4f8ec34c9c56d6e6028f18c68a241f4bbac062925d9e9b7d7e3501d3381bd60f`，内部 `-end-2` 2 处、`pointer-events-none` 2 处、`ml-auto` 0 处。favicon 为 `paperhelper-favicon-light.svg?v=0.1.50`；`https://paper.pilo.eu.cc/` 与 `/zh` 未带会话时 302 到 `/zh/login?redirect=%2F` 再 200（未登录的预期行为）。
  - **版式实测**（Playwright 驱动本机 headless Chromium，脚本放 `/tmp`，仓库内未新增任何 Playwright 依赖或脚本）：`zh`（`dir=ltr`）下书架左栏 `aside.w-72` 宽 **288px**、tab 条 **287px**、四个按钮宽度合计 **287px**，每个标题 `Range.getClientRects()` 行数 **1**、块高 **20px**、按钮高 40px；徽标文本 `99+`，计算样式 `position: absolute`、`inset-inline-end: -8px`，其右边缘比标题包裹层右边缘外扩 8px（`badge.right - wrapper.right = -8px`，四个都成立），四个徽标全部落在 288px 侧栏内。`ar`（`dir=rtl`）下同样是 288 / 287 / 287，每个标题 1 行（块高 16px，阿拉伯文字形高度不同），徽标改为贴标题包裹层的**左**边缘（`badge.left - wrapper.left = -8px`，四个都成立），即随 `dir` 镜像到左上角，且仍全部在侧栏内。可访问名实测为 `所有 (99+)` / `创建 (99+)` / `导入 (99+)` / `收藏 (99+)`。截图：`/tmp/req0126-zh-after.png`、`/tmp/req0126-ar-after.png`。
  - **负对照（必须项）**：在**同一个线上页面**的 DOM 里注入改造前的结构（标题与徽标同为按钮的文档流内子节点、按钮带 `gap-1.5`、徽标 `inline-flex` + `min-width:18px`，外层容器同为 288px），`zh` 下四个标题全部变成 **2 行、块高 40px**（每行 20px）、按钮高 60px —— 正是用户看到的逐字竖排，说明上面这套度量确实抓得住原始缺陷，而不是「改完看着还行」。`ar` 下同一结构不换行而是**横向溢出**（四个按钮宽度合计 367.05px > 288px）：阿拉伯语标签是不带空格的整词、`min-content` 宽度大，`flex-1` 项默认的 `min-width:auto` 使它撑破容器而不折行；这是同一根因在 RTL 下的另一种表现，读数如实记录，不当作「RTL 也复现了逐字竖排」。
- 说明（验收方法的边界，勿误读）：登录态不可得（生产只开着管理员 GitHub 登录，C 端测试账号密码未记录，取不到真实会话），实测用的是 `pr_session=1` cookie（`frontend/src/middleware.ts` 只校验该 cookie 是否存在）+ 注入 localStorage 的假 token + 拦截 `**/api/papers**` 返回 `{"code":0,"data":{"items":[],"total":128,…}}`。**计数是 mock 的**（取 128 是为沿用回归用例里的 `99+` 截断场景）；被测量的 HTML/CSS/JS 全部是线上部署的真实产物，sha256 已逐字节比对。负对照里的 `min-width:18px` 以行内 `style` 提供，因为该类已随旧徽标从构建中移除。
- 遗留与人工事项：① 线上后端仍是 `paper-reader-backend-0.1.49.jar`（有意，见「构建与部署」），下次后端有实际改动时会随新 jar 把版本号带到线上；② `dev` 未跟进本轮（仍为 `b0409ef`，是 `main` 的祖先，v0.1.51 里已快进到与 `main` 相同的提交）；③ 侧栏折叠态的圆形菜单项徽标本轮未改动，也未纳入本次几何验收；④ 版式实测依赖 mock 计数，登录态下用真实计数的截图验收未做；⑤ v0.1.48 / v0.1.49 的遗留（机翻未校对、中文 AI 提示词与后端 `message` 有意保留）不变；⑥ **本节把菜单栏 `Sidebar.tsx` 一并改掉属越界，已由 v0.1.51 回退，见下条**——本节「实现」「测试」「线上验收」里凡涉及 `Sidebar.tsx` / `ml-auto` / 两处 `aria-label` 的表述均已失效。

### v0.1.51 菜单栏计数回退为行内、仅侧栏保留角标（2026-09-17 UTC）

- 需求（`REQ-202609-0126` 的**范围纠正**，不是新需求）：用户指出 v0.1.50 改过头了 —— 「书架 发现 交流，这些地方，这是叫**菜单栏**，菜单栏这里的数字**不要**做成徽标，它是**恢复原来的样子**；只有这些菜单打开之后的**侧栏**，侧栏的菜单才需要做成徽标」。判断依据是**容器形态**而非「统一风格」。
- 先厘清两个同名不同物的左栏（**这是本轮唯一的坑，后续别再混为一谈**）：**菜单栏** = `frontend/src/components/layout/Sidebar.tsx`，全局左栏 `w-[220px]`，按 书架 / 发现 / 交流 分组，共 `library`/`history`/`created`/`notes`/`annotations`/`tags`/`circle`/`chats` 八项，**单列整行**（行宽实测 211px），数字历来是**行尾的行内胶囊**（`<span className="flex-1 text-start">{t(key)}</span>` + `<span className="ms-auto … rounded-[10px]">`，flex 兄弟）；**侧栏** = 点开菜单后左栏里的 `TabBar`（「我的书架」的 所有 / 创建 / 导入 / 收藏，挂在 `components/papers/PaperList.tsx` 与 `components/layout/RightPanel.tsx`），容器 `aside.w-72` 288px，四个 `flex-1` 按钮各约 71.75px。**只有后者**需要角标：窄容器 + 均分按钮时流内徽标会把标题挤成逐字竖排；菜单栏行内可用宽 187px，压根压不着标题，v0.1.50 把同一套改造推过去是判断错误。
- 实现：`Sidebar.tsx` 的 `renderExpandedItem` 回退到 v0.1.50 之前的写法——标签 `<span className="flex-1 text-start">`、计数 `<span className="ms-auto bg-[var(--accent-soft)] … px-[7px] py-[2px] rounded-[10px]">`，并删掉 v0.1.50 加的 `aria-label={badge ? \`${label} (${badge})\` : undefined}`、`relative inline-flex min-w-0 max-w-full` 包装层、内层 `min-w-0 truncate` 与 `aria-hidden` 角标。**回退后与原始版本只差两个类名**：`text-left → text-start`、`ml-auto → ms-auto`，这两处逻辑方向类**必须保留**（14 种语言含 RTL 的 `ar`/`fa`/`ug`）。`frontend/src/components/ui/TabBar.tsx` 与 `frontend/src/test/tabbar-badge.test.tsx` **一字未动**。同时把 `docs/ATTENTION.md` 里把「必须用角标」外推到菜单栏的那段结论改成只适用于侧栏，并明确「窄容器 + 均分」才是判据。
- 测试：**没有给菜单栏加新测试**（本轮目标是恢复原状，加测试反而会把「原状」固化成新契约）；侧栏的 `tabbar-badge.test.tsx`（5 项）原样保留。前端全量 **22 个测试文件 129 项全绿**，`pnpm exec tsc --noEmit`、`next lint`（仅既有告警）、`pnpm run build` 均退出码 0。后端本轮零改动，未重跑。
- 版本链：`8331e47`（回退 + `ATTENTION.md` 更正）→ `f3ff2be`（版本号升到 `0.1.51`）→ `13421c6` 等仅文档的同步提交，均在 `feature/v0.1.51` 并推送；以**显式 `--no-ff` 合并提交** `cd46367`（父提交 `f2f708e` + `f3ff2be`）合入 `main` 并推送，随后把 `dev` 从 `f2f708e` **快进**到 `cd46367`（非强推、未改写历史）；文档同步提交在 `feature/v0.1.51` 上继续追加，再以**第二个显式 `--no-ff` 合并提交**带入 `main`，`dev` 一并快进到该合并提交，至此 `main` 与 `dev` 同指该提交、`feature/v0.1.51` 为其祖先。
- 构建与部署：本轮是**纯前端改动**。前后端 `VERSION`、`frontend/package.json`、`backend/build.gradle.kts`、favicon `?v=` 按发布规范统一到 `0.1.51`；在 `/root/paper-reader/frontend` 执行 `pnpm run build` 后 `pm2 restart paper-reader-frontend`（重启计数 **12 → 13**，2026-09-17 14:22:03 UTC 起 online，进程 id 0 未变；重启前先健康检查、`pm2 save`）。**后端 jar 未重新构建、`paper-reader-backend` 未重启**：线上后端进程 id 11 仍是 2026-09-16 00:37:01 UTC 起跑的 `paper-reader-backend-0.1.49.jar`（`restart_time=0`），`/api/health` 仍返回 `0.1.49`——有意为之，判断前端版本请以 favicon `?v=` 与构建产物哈希为准。
- 线上验收（2026-09-17 UTC，`https://paper.pilo.eu.cc`）：
  - **构建产物逐字节一致**：服务端样式表 `_next/static/css/0f36cad2d495bd20.css` sha256 `276f1a1655d063b823d9b3110ce04b9303ebfd729ffa4c7af36b148319210542`、`_next/static/css/1dfb5e71b60cea90.css` sha256 `8547a52136357ea9a053d09abefac1fb0d08f481e201cd0dc2b3fbff54837642`、`_next/static/chunks/app/%5Blocale%5D/page-0788348c7811f6f2.js` sha256 `f7674fbffcce4c0f71defd6fba6dcf75a4691923985337700248a998a097b2fa`，均与本地 `.next` 完全相同。**同一份 CSS 里两种形态并存**，正是本轮要做的事：`ms-auto`（菜单栏回退后的行内胶囊）、`px-[7px]` / `py-[2px]` / `rounded-[10px]`（胶囊自身）与 `min-w-[16px]`、`.-end-2{…}`（侧栏角标）都在，而 `ml-auto` **0 处**。favicon 为 `paperhelper-favicon-light.svg?v=0.1.51`，页面左栏底部亦显示 `v0.1.51`。
  - **版式实测**（Playwright 驱动本机 headless Chromium，脚本放 `/tmp`，仓库内未新增任何 Playwright 依赖或脚本）：`zh`（`dir=ltr`）下菜单栏 `aside` 宽 **220px**、八个按钮各 211×36.5px，八行的计数胶囊全部 `position: static`、`inset-inline-end: auto`、`margin-inline-start: 0`，且都完全落在按钮内（胶囊右边缘 203px vs 按钮右边缘 215px，差 12px 恰为 `px-3`），与行垂直中心偏差 **0**；`ar`（`dir=rtl`）下同样 static 且胶囊镜像到**行首一侧**（RTL 的 inline-end 在左）。标签行数全部为 1，唯一例外是 `ar` 的「群组私信」（`دردشات المجموعات والرسائل الخاصة`，45 字符）在 211px 行里正常折成 2 行（每行 114.47 / 94.39px）——这是**回退后的原有版式**，不是 v0.1.50 那种逐字竖排（逐字竖排的行宽约 10px，度量里以「行数 > 1 且最窄行 < 40px」为判据，此处两条都不满足）。侧栏 `aside.w-72` 宽 **288px**，四个 tab 各 **71.75px**，角标仍为 `position: absolute`、`inset-inline-end: -8px`、`top: -8px`，标签单行。截图：`/tmp/req0126b-zh-menubar.png`、`/tmp/req0126b-zh-aside.png`、`/tmp/req0126b-ar-*.png`、`-full.png`。
